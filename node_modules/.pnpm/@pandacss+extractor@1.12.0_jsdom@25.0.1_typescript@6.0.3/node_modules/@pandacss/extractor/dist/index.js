...
var getPropertyName = (property, stack, ctx) => {
  ...
  if (import_ts_morph4.Node.isPropertyAssignment(property)) {
    ...
    if (import_ts_morph4.Node.isIdentifier(node)) return box.from(node.getText(), node, stack);
    if (import_ts_morph4.Node.isComputedPropertyName(node)) {
      const expression = node.getExpression();
      stack.push(expression);
      return maybePropName(expression, stack, ctx);
    }
    if (import_ts_morph4.Node.isStringLiteral(node) || import_ts_morph4.Node.isNumericLiteral(node)) return box.from(node.getLiteralText(), node, stack);
  }
  if (import_ts_morph4.Node.isShorthandPropertyAssignment(property)) {
    const name = property.getName();
    if (name != null) return box.from(name, property, stack);
  }
  if (import_ts_morph4.Node.isGetAccessorDeclaration(property)) {
    const node = unwrapExpression(property.getNameNode());
    if (import_ts_morph4.Node.isIdentifier(node)) return box.from(node.getText(), node, stack);
    if (import_ts_morph4.Node.isComputedPropertyName(node)) {
      const expression = node.getExpression();
...
var getObjectLiteralExpressionPropPairs = (expression, expressionStack, ctx, matchProp) => {
  ...
  properties.forEach((property) => {
    ...
    stack.push(property);
    if (import_ts_morph5.Node.isPropertyAssignment(property) || import_ts_morph5.Node.isShorthandPropertyAssignment(property) || import_ts_morph5.Node.isGetAccessorDeclaration(property)) {
      ...
      let init;
      if (import_ts_morph5.Node.isGetAccessorDeclaration(property)) {
        const body = property.getBody();
        init = import_ts_morph5.Node.isBlock(body) ? body.getStatements().at(-1) : void 0;
      } else {
        init = property.getInitializer();
      }
      if (!init) return;
      const returnExpression = import_ts_morph5.Node.isReturnStatement(init) ? init.getExpression() : void 0;
...
function maybeBoxNode(node, stack, ctx, matchProp) {
  ...
  if (import_ts_morph6.Node.isCallExpression(node)) {
    if (ctx.tokens) {
      ...
      if (import_ts_morph6.Node.isIdentifier(expr)) {
        fnName = expr.getText();
      } else if (import_ts_morph6.Node.isPropertyAccessExpression(expr)) {
        const exprName = expr.getExpression();
        const propertyName = expr.getName();
        if (import_ts_morph6.Node.isIdentifier(exprName)) {
      ...
      if (isTokenFunction) {
        ...
        if (tokenPathArg && import_ts_morph6.Node.isStringLiteral(tokenPathArg)) {
          const tokenPath = tokenPathArg.getLiteralValue();
          const fallbackArg = args[1];
          const fallbackValue = fallbackArg && import_ts_morph6.Node.isStringLiteral(fallbackArg) ? fallbackArg.getLiteralValue() : void 0;
...
var maybeBindingElementValue = (def, stack, propName, ctx) => {
  ...
  if (import_ts_morph6.Node.isArrayBindingPattern(parent)) {
    ...
    if (import_ts_morph6.Node.isVariableDeclaration(grandParent)) {
      ...
      if (!import_ts_morph6.Node.isArrayLiteralExpression(initializer)) return;
...
function getNameLiteral(wrapper) {
  if (import_ts_morph6.Node.isStringLiteral(wrapper)) return wrapper.getLiteralText();
...
var getElementAccessedExpressionValue = (expression, _stack, ctx) => {
  ...
  if (import_ts_morph6.Node.isBinaryExpression(arg)) {
    if (arg.getOperatorToken().getKind() !== import_ts_morph6.ts.SyntaxKind.PlusToken) return;
...
var collectImports = (sourceFile) => {
  ...
  const resolveBundledNamespaceMod = (node) => {
    ...
    if (import_ts_morph7.Node.isCallExpression(expression)) {
      const callee = unwrapExpression(expression.getExpression());
      if (!import_ts_morph7.Node.isIdentifier(callee)) return;
  ...
  sourceFile.getDescendantsOfKind(import_ts_morph7.SyntaxKind.CallExpression).forEach((callExpression) => {
    const callee = unwrapExpression(callExpression.getExpression());
    if (!import_ts_morph7.Node.isIdentifier(callee) || callee.getText() !== "parcelRegister") return;
    const [idArg, factoryArg] = callExpression.getArguments();
    if (!import_ts_morph7.Node.isStringLiteral(idArg)) return;
    if (!import_ts_morph7.Node.isArrowFunction(factoryArg) && !import_ts_morph7.Node.isFunctionExpression(factoryArg)) return;
  ...
  sourceFile.getVariableDeclarations().forEach((declaration) => {
    if (!import_ts_morph7.Node.isIdentifier(declaration.getNameNode())) return;
    ...
    if (!mod && initializer && import_ts_morph7.Node.isCallExpression(initializer)) {
      const callee = unwrapExpression(initializer.getExpression());
      const [firstArg] = initializer.getArguments();
      if (import_ts_morph7.Node.isIdentifier(callee) && callee.getText() === "parcelRequire" && import_ts_morph7.Node.isStringLiteral(firstArg)) {
        mod = parcelRuntimeModuleIds.get(firstArg.getLiteralValue());
      }
    }
    if (!mod && initializer && import_ts_morph7.Node.isObjectLiteralExpression(initializer)) {
      const propertyNames = new Set(
        initializer.getProperties().map((property) => {
          if (import_ts_morph7.Node.isPropertyAssignment(property)) {
  ...
  sourceFile.getDescendantsOfKind(import_ts_morph7.SyntaxKind.FunctionDeclaration).forEach((declaration) => {
  ...
  const resolveBundledAliasImport = (node) => {
    if (!node) return;
    const expression = unwrapExpression(node);
    if (import_ts_morph7.Node.isIdentifier(expression)) {
      return bundledNamed.get(expression.getText());
    }
    if (import_ts_morph7.Node.isConditionalExpression(expression)) {
      return resolveBundledAliasImport(expression.getWhenTrue()) ?? resolveBundledAliasImport(expression.getWhenFalse());
    }
    if (import_ts_morph7.Node.isBinaryExpression(expression) && expression.getOperatorToken().getKind() === import_ts_morph7.SyntaxKind.CommaToken) {
...
var getTagName = (node) => {
  if (!node) return;
  const expression = unwrapExpression(node);
  if (import_ts_morph7.Node.isStringLiteral(expression) || import_ts_morph7.Node.isNoSubstitutionTemplateLiteral(expression)) {
    return expression.getLiteralValue();
  }
  if (import_ts_morph7.Node.isIdentifier(expression) || import_ts_morph7.Node.isPropertyAccessExpression(expression) || import_ts_morph7.Node.isElementAccessExpression(expression)) {
...
var createCompiledJsxContext = (sourceFile) => {
  ...
  const resolveIdentifierDefinition = (identifier) => {
    ...
    if (import_ts_morph7.Node.isFunctionDeclaration(declaration)) {
    ...
    if (import_ts_morph7.Node.isVariableDeclaration(declaration)) {
  ...
  const resolveCallee = (node) => {
    ...
    if (import_ts_morph7.Node.isPropertyAccessExpression(expression)) {
      const target = normalizeCallee(expression.getExpression());
      if (!import_ts_morph7.Node.isIdentifier(target)) return;
  ...
  const isMergePropsCall = (node) => {
    if (!import_ts_morph7.Node.isCallExpression(node)) return false;
...
// src/extract.ts
var isImportOrExport = (node) => import_ts_morph9.Node.isImportDeclaration(node) || import_ts_morph9.Node.isExportDeclaration(node);
var isJsxElement = (node) => import_ts_morph9.Node.isJsxOpeningElement(node) || import_ts_morph9.Node.isJsxSelfClosingElement(node);
var extract = ({ ast, ...ctx }) => {
  ...
  const processCompiledPropSource = (componentNode, component, boxByProp, propNode, matchProp) => {
    ...
    if (import_ts_morph9.Node.isObjectLiteralExpression(expression)) {
    ...
    if (import_ts_morph9.Node.isCallExpression(expression) && compiledJsx.isMergePropsCall(expression)) {
  ...
  ast.forEachDescendant((node, traversal) => {
    ...
    if (components) {
      if (import_ts_morph9.Node.isJsxOpeningElement(node) || import_ts_morph9.Node.isJsxSelfClosingElement(node)) {
      ...
      if (import_ts_morph9.Node.isJsxSpreadAttribute(node)) {
      ...
      if (import_ts_morph9.Node.isJsxAttribute(node)) {
        const componentNode = node.getFirstAncestor(isJsxElement);
        const component = componentByNode.get(componentNode);
        if (!componentNode || !component) return;
        const componentName = getComponentName(componentNode);
      ...
      if (import_ts_morph9.Node.isCallExpression(node)) {
    ...
    if (functions && import_ts_morph9.Node.isCallExpression(node)) {
      const expr = node.getExpression();
      const fnName = import_ts_morph9.Node.isCallExpression(expr) ? expr.getExpression().getText() : expr.getText();
    ...
    if (taggedTemplates && import_ts_morph9.Node.isTaggedTemplateExpression(node)) {
      const tag = node.getTag();
      const fnName = import_ts_morph9.Node.isCallExpression(tag) ? tag.getExpression().getText() : tag.getText();
  ...
  componentByNode.forEach((parentRef, componentNode) => {
    const component = componentByNode.get(componentNode);
    if (!component) return;
    if (import_ts_morph9.Node.isCallExpression(componentNode) && component.props.size === 0 && component.conditionals.length === 0)
      return;
    const query = {
      name: parentRef.name,
...
var extractJsxElementProps = (node, ctx) => {
  ...
  jsxAttributes.forEach((attrNode) => {
    if (import_ts_morph10.Node.isJsxAttribute(attrNode)) {
    ...
    if (import_ts_morph10.Node.isJsxSpreadAttribute(attrNode)) {
...
var getLiteralValue = (node, ctx) => {
  ...
  if (box.isConditional(node)) {
    ...
    if (last && import_ts_morph11.Node.isArrayLiteralExpression(last) && !Number.isNaN(maybeIndex)) {
