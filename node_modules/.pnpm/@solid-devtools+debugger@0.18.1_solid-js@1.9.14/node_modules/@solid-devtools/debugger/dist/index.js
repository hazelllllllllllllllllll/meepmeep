import { $SDT_ID, defaultWalkerMode, LOCATION_ATTRIBUTE_NAME, WINDOW_PROJECTPATH_PROPERTY, UNDEFINED, INFINITY, NEGATIVE_INFINITY, NAN } from './chunk-H25EBNZF.js';
export { $SDT_ID, INFINITY, LOCATION_ATTRIBUTE_NAME, MARK_COMPONENT, NAN, NEGATIVE_INFINITY, NODE_TYPE_NAMES, NodeType, PropGetterState, TreeWalkerMode, UNDEFINED, USE_LOCATOR, ValueItemType, ValueType, WINDOW_PROJECTPATH_PROPERTY, defaultWalkerMode, getValueItemId } from './chunk-H25EBNZF.js';
import { trimString, warn, whileArray, asArray, isRecord } from '@solid-devtools/shared/utils';
import { throttle, scheduleIdle } from '@solid-primitives/scheduled';
import { untrack, getOwner as getOwner$1, runWithOwner, createRoot, createComputed, createMemo, batch, onCleanup, createSignal, createEffect, Index, Show, $PROXY, getListener } from 'solid-js';
...
var flushRootUpdateQueue = throttle(forceFlushRootUpdateQueue, 250);
...
var findOwnerById = (rootId, id) => {
  ...
  if (!root)
    return;
  return whileArray([root], (owner, toCheck) => {
...
function createLocator({
  ...
  const devtoolsTarget = atom(null);
  ...
  const calcHighlightedComponents = (target) => {
    ...
    if (target.type === "componentNode") {
      ...
      if (!comp2)
        return [];
      return asArray(comp2.elements).map((element) => ({
        element,
        id: componentId,
        name: comp2.name
      }));
...
function createInspector(debuggerEnabled, { eventHub }) {
  ...
  createEffect(() => {
    if (!debuggerEnabled())
      return;
    onCleanup(() => setInspectedOwner(void 0));
...
var plugin_default = createInternalRoot(() => {
  ...
  const [debuggerEnabled, setUserEnabledSignal, setLocatorEnabledSignal] = (() => {
    const locatorEnabledSignal = atom();
    const userEnabledSignal = atom();
    const combinedEnabled = atom(false);
    createComputed(
      defer(
