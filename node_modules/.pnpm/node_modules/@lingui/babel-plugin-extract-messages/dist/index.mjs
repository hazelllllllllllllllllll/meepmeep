...
function getLinguiConfig(ctx) {
  const { linguiConfig } = ctx.opts;
  if (linguiConfig) return linguiConfig;
  const loadedConfig = ctx.get("linguiConfig");
...
function index({ types: t }) {
  function isTransComponent(path, config) {
    const [moduleName, importName] = config.runtimeConfigModule.Trans;
    return path.isJSXElement() && path.get("openingElement").get("name").referencesImport(moduleName, importName);
  }
