...
var accessor = (a) => typeof a() === "function" ? a() : a.bind(void 0);
var toFunction = (a) => (...args) => {
  const v = a();
...
function repeat(times, mapFn, options = {}) {
  let disposers = [], items = [], prevLen = 0;
  solidJs.onCleanup(() => disposers.forEach((f) => f()));
  const mapLength = (len) => {
    if (len === 0) {
    ...
    if (prevLen === 0) {
      if (disposers[0])
        disposers[0]();
      for (let i = 0; i < len; i++)
        items[i] = solidJs.createRoot(mapper.bind(void 0, i));
      return items;
    }
    {
  ...
  const mapper = (index, dispose) => {
    disposers[index] = dispose;
    return mapFn(index);
  };
  const memoLen = solidJs.createMemo(() => Math.floor(Math.max(times(), 0)));
  return () => {
    const len = memoLen();
...
function mapRange(getStart, getTo, getStep, mapFn, options = {}) {
  ...
  solidJs.onCleanup(() => disposers.forEach((f) => f()));
  const mapper = (i, n, itemsList, disposersList) => solidJs.createRoot((dispose) => {
  ...
  const mapNewRange = (start, to, step) => {
    ...
    const newDisposers = new Array(newLength);
    const oldDisposers = disposers;
    let end;
    if (start < prevStart) {
  ...
  return () => {
    ...
    const list = solidJs.untrack(mapNewRange.bind(void 0, _start, _to, _step));
    prevStart = _start;
    prevTo = _to;
    prevStep = _step;
...
function indexRange(getStart, getTo, getStep, mapFn, options = {}) {
  ...
  solidJs.onCleanup(() => disposers.forEach((f) => f()));
  const mapper = (i, n) => solidJs.createRoot((dispose) => {
    const [number, setNumber] = solidJs.createSignal(n);
    disposers[i] = dispose;
  ...
  return () => {
    ...
    return solidJs.untrack(mapNewRange.bind(void 0, _start, _to, _step));
  };
}
function IndexRange(props) {
  let start, to, step;
  if ("to" in props) {
    start = () => props.start ?? 0;
    to = () => props.to;
