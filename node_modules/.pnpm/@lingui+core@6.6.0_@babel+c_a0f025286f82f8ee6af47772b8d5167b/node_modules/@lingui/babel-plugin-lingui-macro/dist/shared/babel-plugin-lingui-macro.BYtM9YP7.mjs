...
function createMessageDescriptorFromTokens(tokens, oldLoc, descriptorFields, defaults = {}, transforms = {}) {
...
function createMessageDescriptor(result, oldLoc, descriptorFields, defaults = {}) {
  ...
  properties.push(
    explicitIdProperty ? explicitIdProperty : createIdProperty(
      message,
...
function createExplicitIdProperty(defaults) {
  if (!defaults.id) {
    return;
  }
  const explicitId = isObjectProperty(defaults.id) ? getTextFromExpression(defaults.id.value) : defaults.id.text;
  const resolvedId = explicitId !== void 0 && defaults.idPrefix && (!defaults.idPrefixLeader || explicitId.startsWith(defaults.idPrefixLeader)) ? defaults.idPrefix + explicitId : explicitId;
...
function createValuesProperty(key, values) {
  const valuesObject = Object.keys(values).map(
    (key2) => t.objectProperty(
      t.isValidIdentifier(key2) ? t.identifier(key2) : (
        ...
        /^\d+$/.test(key2) ? t.numericLiteral(Number(key2)) : t.stringLiteral(key2)
      ),
      values[key2]
    )
  );
...
function createMacroJsContext(isLinguiIdentifier2, descriptorFields, getDirective = () => void 0) {
  return {
...
function tokenizeTemplateLiteral(node, ctx) {
  ...
  return tpl.quasis.flatMap((text, i) => {
    ...
    if (currExp) {
      argTokens = t.isCallExpression(currExp) || t.isTaggedTemplateExpression(currExp) ? tokenizeNode(currExp, false, ctx) : [tokenizeExpression(currExp, ctx)];
    }
    const textToken = {
      type: "text",
      value
...
export { ICUMessageFormat as I, JsMacroName as J, MsgDescriptorPropKey as M, createMessageDescriptor as a, createMessageDescriptorFromTokens as b, createMacroJsContext as c, createStringObjectProperty as d, isChoiceMethod as e, isDefineMessage as f, isI18nMethod as g, isLinguiIdentifier as h, isArgDecorator as i, tokenizeChoiceComponent as j, tokenizeExpression as k, tokenizeNode as l, tokenizeTemplateLiteral as m, makeCounter as n, JsxMacroName as o, processDescriptor as p, tokenizeArg as t };
