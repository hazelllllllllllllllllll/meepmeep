...
function generateTsupOptions(options, generate_options = {}) {
  const items = [];
  ...
  return items.map((item, i) => {
    ...
    for (const entry of entries) {
      const export_path = type.dev ? entry.exports.dev : type.server ? entry.exports.server : entry.exports.main;
      const entry_path = type.dev && typeof entry.options.dev_entry === "string" ? entry.options.dev_entry : type.server && typeof entry.options.server_entry === "string" ? entry.options.server_entry : entry.options.entry;
    ...
    return {
      ...
      esbuildOptions(es_options) {
        ...
        if (!type.dev && options.drop_console)
          es_options.drop = ["console", "debugger"];
