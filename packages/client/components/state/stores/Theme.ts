...
export type TypeTheme = {
  ...
  /**
   * Which Catppuccin flavour is active
   */
...
export class Theme extends AbstractStore<"theme", TypeTheme> {
  ...
  default(): TypeTheme {
    return {
      ...
      // Default accent changed to a Catppuccin blue
      m3Accent: "#89B4FA",
      m3Contrast: 0.0,
  ...
  clean(input: Partial<TypeTheme>): TypeTheme {
    ...
    if (
      ["latte", "frappe", "macchiato", "mocha"].includes(
        input.catppuccinFlavor!,
      )
    ) {
      data.catppuccinFlavor = input.catppuccinFlavor!;
    }
    ...
    switch (opts.preset) {
      case "you":
        return {
          ...
          catppuccinFlavor: opts.catppuccinFlavor,
  ...
  /**
   * Get current Catppuccin flavour
   */
  get catppuccinFlavor() {
    return this.get().catppuccinFlavor;
  }

  /**
   * Set the active Catppuccin flavour
   * @param flavor Catppuccin flavour
   */
  setCatppuccinFlavor(flavor: CatppuccinFlavor) {
    this.set("catppuccinFlavor", flavor);
  }
